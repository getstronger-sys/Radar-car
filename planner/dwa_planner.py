...
class DWAPlanner:
    def __init__(self, config):
        ...
        根据是否导入了 PythonRobotics，选择使用官方封装的配置类还是自定义的简单参数表
        后者是备用方案。
        """
        if PYTHONROBOTICS_AVAILABLE:
            ...
            self.config.obstacle_cost_gain = config.get('obstacle_cost_gain', 0.5)
            ...
            self.obstacle_cost_gain = config.get('obstacle_cost_gain', 0.5)
    ...
    def plan_simple(self, current_state, current_velocity, goal, occupancy_grid):
        ...
        def inflate_obstacles(occupancy_grid):
            ...
            return inflated, dilation_radius
        ...
        def calc_obstacle_cost(traj, inflated_map):
            """计算障碍物代价"""
            # 如果地图全为0（无障碍），直接返回0.0
            if np.count_nonzero(inflated_map) == 0:
                return 0.0
        ...
        # 膨胀障碍物
        inflated_map, dilation_radius = inflate_obstacles(occupancy_grid)
