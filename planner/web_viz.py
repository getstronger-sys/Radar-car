...
with st.sidebar:
    ...
    for i in range(robot_num):
        ...
        sx = st.number_input(f"起点X{i+1}", 0.0, float(map_size_m), float(0.5 + i))
        sy = st.number_input(f"起点Y{i+1}", 0.0, float(map_size_m), float(0.5 + i))
        gx = st.number_input(f"终点X{i+1}", 0.0, float(map_size_m), float(map_size_m-0.5-i))
        gy = st.number_input(f"终点Y{i+1}", 0.0, float(map_size_m), float(map_size_m-0.5-i))
...
with tab2:
    ...
    for step in range(100):
        ...
        accel = [
            (new_velocity[0] - robot_velocity[0]) / dt,
            (new_velocity[1] - robot_velocity[1]) / dt
        ]
        accels.append(accel)
